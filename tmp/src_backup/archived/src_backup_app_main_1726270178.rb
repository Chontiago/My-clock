...
class TimeAnimation
  ...
  def option_select #Depende de animation_option 1, depende de mouse_click 1
    ...
    @option_select.text_clicked.each do |animation|
      if mouse_click(animation)
        @selected_animation = animation.text
      end
    end
    
  end

  def selected_animation #Depende de option_select 2, depende de current_animation 0 
