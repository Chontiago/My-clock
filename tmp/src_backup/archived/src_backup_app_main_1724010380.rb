...
class TimeAnimation
  def initialize(args)
    ...
    @clock_slide_animation ||= Slide.new(@clock)
  ...
  class Slide
    def initialize(sliding_object, *borders)
      ...
      @horizontal_borders = borders
      @reverse ||= false
      @bottom_limit = Borders.new(0,(0 - @clock.height), SCREEN_WIDTH, 0, "Vertical")
      @top_limit = Borders.new(0, SCREEN_HEIGHT + @clock.height, SCREEN_WIDTH, 0, "Vertical")
      @vertical_limits = [@bottom_limit, @top_limit]
    ...
    def vertical_screen_movement(object)
        @vertical_limits.each do |limits| #initialize
         if collision(limits, object)
           object.vertical_speed *= REVERSE_TRAYECTORY
             if @reverse == false
  ...
  def slide_animation
    ...
    clock_reached_vertical_limit_allignement(@clock) #object_reached_vertical_limit
    @clock_slide_animation.vertical_limits
    horizontal_screen_movement(@clock)
  ...
  def clock_reached_vertical_limit_allignement(object)
    object.x_pos = [object.x_pos, object.x_pos].max
