...
class TimeAnimation
  ...
  class Clock
    ...
    def current_time
      return  {x: @x_pos, y: @y_pos, anchor_x: @anchor, anchor_y: @anchor, text: @time}
    end
    
    def clock_borders
      return  {x: @x_pos , y: @y_pos - 20, w: @width, h: @height} #cambiar a ligarlo con el width del texto 
    end
  end
  ...
  def clock
    current_hour_and_minutes = Text.new("#{@time.hour} : #{@time.min}", x: HALF_SCREEN_HEIGHT, y: HALF_SCREEN_HEIGHT, size: 32, color: [0,0,0])
    
    @clock ||= Clock.new(HALF_SCREEN_WIDTH, HALF_SCREEN_HEIGHT, 50, 50, "#{@time.hour} : #{@time.min}")
  end
