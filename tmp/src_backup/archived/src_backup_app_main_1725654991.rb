...
=end
  ...
  def randomize_color_2
    ...
    @full_clock.each_with_index do |elements, index|
      elements.color.each_with_index do |colors, sub_index| 
        if @counter_index == elements[index]
          elements_index.color[sub_index] = rand(255)
        end
      end
    end
  ...
  def debug 
    debug_text =  ["#{@invert}", "#{@index_counter}" ]
