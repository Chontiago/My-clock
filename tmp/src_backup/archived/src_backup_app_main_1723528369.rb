...
class TimeAnimation
  ...
  def screen_borders
    ...
    bottom_border = Borders.new(0, 0, SCREEN_WIDTH, 10, "Vertical")
  ...
  def render_clock
    ...
    bottom_border = Borders.new(0, 0, SCREEN_WIDTH, 10, "Vertical" ).set_borders
    @args.outputs.solids << bottom_border
  ...
  def bounce_movement(border)
    reverse_trayectory = -1.0


    if border.bounce_angle == "Vertical"
      @clock.vertical_speed *= reverse_trayectory
    else
      @clock.horizontal_speed *= reverse_trayectory
  ...
  def collision(box1, box2) #box1 ball, box2 object
    ...
    box1.y_pos + box1.height >= box1.y_pos &&
