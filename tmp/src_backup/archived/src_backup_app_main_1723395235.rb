...
class TimeAnimation
  ...
  class Clock
    ...
    def current_time
      return  {x: @x_pos, y: @y_pos, anchor_x: @anchor, anchor_y: @anchor, text: @text}
    end
  ...
  def clock
    ...
    @clock ||= Clock.new(HALF_SCREEN_WIDTH, HALF_SCREEN_HEIGHT, 50, 50, "hola")
  end

  def render_clock
    @args.outputs.solids << {x: HALF_SCREEN_WIDTH, y: HALF_SCREEN_HEIGHT, anchor_x: 1.5, anchor_y: 1.5, text: "hola" }
  end
