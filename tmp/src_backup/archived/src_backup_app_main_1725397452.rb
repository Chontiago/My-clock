...
class TimeAnimation
  ...
  def expand_animation(animated_object)
    expand_and_retract(animated_object)
    size_limits(animated_object)
  end

  def expand_and_retract(object)
    @expand ||= true
  ...
  def debug 
    Text.new(@args, HALF_SCREEN_WIDTH, 600, 15, "#{@expand}").single_line
  end
