class Slide
  ...
  def initialize(animated_object, side_bounds, main)
    ...
    @bottom_bound = Borders.new(0,(0 - @animated_object.height), SCREEN_WIDTH, 0, "Vertical")
    @top_bound = Borders.new(0, SCREEN_HEIGHT + @animated_object.height, SCREEN_WIDTH, 0, "Vertical")
  end
  ...
  def slide_movement
    ...
    vertical_bounds.each do |bounds|
      if @main.collision(bounds, @animated_object)
