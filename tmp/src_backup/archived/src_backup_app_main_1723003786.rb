...
class TimeAnimation
  def initialize(args)
    @args = args
    @test ||= Text.new(@args, HALF_SCREEN_WIDTH, HALF_SCREEN_HEIGHT, "HOLA" ).single_line
  end
  ...
  class Text 
    ...
    def single_line
      return {x: @x, y: @y, anchor_x: @anchor, anchor_y: @anchor,  text: @text }
    end
