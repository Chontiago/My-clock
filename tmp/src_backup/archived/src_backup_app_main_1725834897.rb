...
class TimeAnimation
  def initialize(args)
    ...
    @clock_color_change_animation ||= ColorChange.new(@full_clock, self)
  ...
  class ColorChange
    ...
    def initialize(animated_array, main)
    ...
    def current_element
      if @main.@args.state.tick_count % 60 == 0
        if @invert_animation
          @current_element -= 1
        else
          @current_element +=1
    ...
    def current_element_color_change
      @animated_array.each_with_index do |elements, index|
        if @current_element == index +1 && @main.@args.state.tick_count % 60 == 0
