...
class TimeAnimation
  ...
  def slide_animation
    move_to_initial_position(@clock)
    slide_movement(@clock)
  end

  def move_to_initial_position(object)
    started_animation ||= false
    if started_animation == false
      started_animation = true
      object.x_pos = 0 
      object.y_pos ||= SCREEN_HEIGHT
    end
    
  end

  def slide_movement(object)
