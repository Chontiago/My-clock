...
class TimeAnimation
  ...
  class Borders
    def initialize(x_pos, y_pos, w, h)
      attr_accessor :x_pos, :y_pos, :width, :h :height
      @x_pos = x_pos
      @y_pos = y_pos
      @width = w
      @height= h
  ...
  class Text 
    def initialize(args, x_pos, y_pos , text ,skip_space=50)
    ...
    def multiple_lines
      @text.each_with_index do |text, index|
        @args.outputs.labels << {x: @x, y:@y + (index * @skip_space) , anchor_x: @anchor, anchor_y: @anchor, text: text}
      end
    end
  ...
  def render_time
    ...
    @time_borders ||= Rectangle.new(HALF_SCREEN_WIDTH, HALF_SCREEN_HEIGHT, 100, 100)
    @time_screen ||= Text.new(@args, HALF_SCREEN_WIDTH, HALF_SCREEN_HEIGHT, current_time)

    @args.outputs.solids << @time_borders
