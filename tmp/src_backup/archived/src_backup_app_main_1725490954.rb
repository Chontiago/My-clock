...
class TimeAnimation
  def initialize(args)
    ...
    @clock_minutes = Clock.new(HALF_SCREEN_WIDTH + @clock_hour.width, HALF_SCREEN_HEIGHT + 50, "#{@time.min}")
    @clock_colon = Clock.new(HALF_SCREEN_WIDTH, HALF_SCREEN_HEIGHT + 50, ":")
  ...
  def render_clock
    ...
    @args.outputs.labels << @clock_minutes.current_time
    @args.outputs.labels << @clock_hour.current_time
    @args.outputs.labels << @clock_colon.current_time
