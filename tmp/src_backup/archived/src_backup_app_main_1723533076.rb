...
class TimeAnimation
  ...
  def bounce_animation
    bounce_animation_initial_movement(@clock)
    bounce(screen_borders, @clock)
  end
  ...
  def bounce(bounce_angle, object)

    bounce_angle.each do |border|
      if collision(bounce_angle, object)
        bounce_trayectory(bounce_angle, object)
      end
    end

  end

  def bounce_animation_initial_movement(object)
    object.x_pos += object.horizontal_speed
