...
class TimeAnimation
  def initialize(args)
    ...
    @clock_2 = Clock.new(HALF_SCREEN_WIDTH, HALF_SCREEN_HEIGHT, "#{@clock_hour.time}" + "#{@clock_colon.time}" + "#{@clock_minutes.time}")
    @full_clock = [@clock_hour, @clock_colon, @clock_minutes]
    @clock = Clock.new(HALF_SCREEN_WIDTH, HALF_SCREEN_HEIGHT, "#{@full_clock[0]}" + "#{@full_clock[1]}" + "#{@full_clock[2]}")
    @clock_array ||= [@clock]
