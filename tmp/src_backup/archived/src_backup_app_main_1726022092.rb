...
class TimeAnimation
  ...
  def mouse_click
    ...
    test = {x: HALF_SCREEN_WIDTH, y: HALF_SCREEN_WIDTH, w: 100, h: 100}

    @args.outputs.solids << test
  ...
  def mouse_point(box2)
    
    mouse = @args.inputs.mouse.point

    mouse[:x] >= box2[:x] && mouse[:x] <= box2[:x] + box2[:w] &&
    mouse[:y] >= box2[:y] && mouse[:y]  <= box2[:y] + box2[:h]
