...
class TimeAnimation
  def initialize(args)
    ...
    @clock2 = Clock.new(HALF_SCREEN_WIDTH, HALF_SCREEN_HEIGHT, "#{@clock_hour.time}" + "#{@clock_colon.time}" + "#{@clock_minutes.time}")
    @full_clock = [@clock_hour, @clock_colon, @clock_minutes]
    @clock = Clock.new(HALF_SCREEN_WIDTH, HALF_SCREEN_HEIGHT, "#{@full_clock}")
    @clock_array ||= [@clock]
