...
class TimeAnimation
  ...
  def important_functions # rename
    clock_objects
    clock_animations
    option_select
  end
  ...
  def options
    ...
    options ||= options_retracted

    options_retracted = [options]    
    options_expanded = {"Bounce" => @clock_bounce, "Slide" => @clock_slide, "Expand" => @clock_expand, "Color Change" => @clock_color_change}
  ...
  def option_select
    @option_select = Text.new(@args, 1, 100, 10, options.keys)
    @selected_option ||= ''    

    @option_select.hitbox.each do |option|
