...
class TimeAnimation
  ...
  def render_time
    ...
    @time_screen_borders ||= Rectangle.new(HALF_SCREEN_WIDTH, HALF_SCREEN_HEIGHT, 30, 70) # cambiar valores más adelante 
    @time_screen ||= Text.new(@args,@time_screen_borders.x_pos, @time_screen_borders.y_pos + 20, current_time)
    

    @time_screen.multiple_lines
    @args.outputs.solids << @time_screen_borders.rectangle_size
