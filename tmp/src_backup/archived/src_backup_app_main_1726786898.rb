...
#Arreglar tema en bounce y slide 
...
class TimeAnimation
  def initialize(args)
    ...
    @clock_array ||= [Clock.new(HALF_SCREEN_WIDTH, HALF_SCREEN_HEIGHT, "#{@clock_hour.time}" + "#{@clock_colon.time}" + "#{@clock_minutes.time}")]
    @reset_clock ||= Clock.new(HALF_SCREEN_WIDTH, HALF_SCREEN_HEIGHT, "#{@clock_hour.time}" + "#{@clock_colon.time}" + "#{@clock_minutes.time}")
  ...
  def clock_animations #Creates an animation for clock, using the Classes for each animation
    ...
    #@clock_bounce ||= Bounce.new(@clock_array, [@left_border, @right_border, @top_border, @bottom_border], self) 
    #@clock_slide ||= Slide.new(@clock_array, [@left_border, @right_border], self)
    @clock_expand ||= Expand.new(@clock_array)
