...
class TimeAnimation
  ...
  def randomize_color
    @clock.color.each do |color|
      color = random_color(color)
    end

    random_color(@clock.color)

    #@clock.color[0] = rand(255)
    #@clock.color[1] = rand(255)
    #@clock.color[2] = rand(255)
    

    random_color = rand(255)
    random_color_2 = rand(255)
    random_color_3 = rand(255)
    random_colors = [rand(255)]

   # @clock.red = random_color(@clock.red)
    #@clock.green = random_color(@clock.green)
    #@clock.blue = random_color(@clock.blue)
    #random_color(@clock.blue)
   

    
    #@clock.red = @clock.red * 0 + rand(255)
    #@clock.green = @clock.green * 0 + rand(255)
    #@clock.blue = @clock.blue * 0 + rand(255)
  
  end

  def random_color(color)
    return color[0] * 0 + rand(255) 
  end
