...
#Make length adjustment in clock class more clear
...
class TimeAnimation
  ...
  class Clock
    ...
    def initialize(x, y, w, h, time)
      ...
      @time = time
      @size = 30
      length_adjustment = @size * 0.15
      @width = ((@time.length - length_adjustment)  * 10) + (@time.length * @size)
      @height = 20 + (@size * 2)
  ...
  def clock
    ...
    @clock ||= Clock.new(@HALF_SCREEN_WIDTH, HALF_SCREEN_HEIGHT, 50, 50, "#{@time.hour} : #{@time.min} PMABC")
  end

  def render_clock
    @args.outputs.labels << @clock.current_time
    @args.outputs.borders << @clock.clock_borders
  end
