class Slide
  ...
  def initial_movement
    @animated_object.y_pos -= @sliding_object.vertical_speed 
  end
  ...
  def side_bounds_reached
    @animated_object.x_pos = [@sliding_object.x_pos, @left_bound.x_pos].max
    @animated_object.x_pos = [@sliding_object.x_pos, (@right_bound.x_pos - @sliding_object.width )].min
  end
