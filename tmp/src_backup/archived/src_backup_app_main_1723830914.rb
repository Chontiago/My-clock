...
class TimeAnimation
  ...
  def screen_borders
    @left_border = Borders.new(0, 0, 0, SCREEN_HEIGHT, "Horizontal").set_borders
    @right_border = Borders.new(SCREEN_WIDTH, 0, 0, SCREEN_HEIGHT, "Horizontal").set_borders
    @bottom_border = Borders.new(0, 50, SCREEN_WIDTH, 10, "Vertical").set_borders
    
    @top_border = Borders.new(0, SCREEN_HEIGHT, SCREEN_WIDTH, 0, "Vertical").set_borders
  ...
  def screen_limits(object)
   
    screen_borders.each do |border|
      if inside_screen(border, @clock.clock_hitbox)
        @args.outputs.labels << {x: HALF_SCREEN_WIDTH, y: HALF_SCREEN_HEIGHT, anchor_x: 1.5, anchor_y: 1.5, text: "ahhhhh"}
        puts "ahhhh"
      end
    end
  ...
  def inside_screen(box1, box2) #box1 border, #box2 Clocl
